...
<xsl:template match="/castxml2pony/renderuses/renderuse[@render!='0']">
	<xsl:variable name="iid" select="@id"/>
  <xsl:variable name="dir" select="@render"/>
  <xsl:variable name="filename" select="/castxml2pony/CastXML/Function[@id=$iid]/@name"/>
  <xsl:result-document href="uses/{$dir}/{$filename}.use" method="text">
