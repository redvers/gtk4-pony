...
use @g_object_class_install_property[None](oclass: NullablePointer[GObjectClassS] tag, propertyid: U32, pspec: NullablePointer[GParamSpecS] tag)
use @g_param_spec_string[NullablePointer[GParamSpecS]](name: Pointer[U8] tag, nick: Pointer[U8] tag, blurb: Pointer[U8] tag, default_value: Pointer[U8] tag, flags: I32)
...
use @g_object_get_property[None](gobj: NullablePointer[PRowEntryS] tag, name: Pointer[U8] tag, gvalue: GValueS)
...
class PRowEntry is GObjectInterface
  ...
  fun @create_properties(gobj: PRowEntryClassS) =>
    @printf("In class_init\n".cstring())
    gobj.parent_class.set_property = PRowEntryP~set_property()
    gobj.parent_class.get_property = PRowEntryP~get_property()
    let g: NullablePointer[GParamSpecS] = @g_param_spec_string[NullablePointer[GParamSpecS]]("name".cstring(), "nick".cstring(), "blurb".cstring(), "wibble".cstring(), I32(3))
    try
      @printf("class_init_checking_gparamspecs: %d\n".cstring(),
        g.apply()?.value_type)
    end
    @g_object_class_install_property(NullablePointer[GObjectClassS](gobj.parent_class), U32(1), g)
...
//    gv.set_pointer[String val](consume st)
    @g_object_get_property(NullablePointer[PRowEntryS](ptr), name.cstring(), gv.ptr)
