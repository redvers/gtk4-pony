...
use @gtk_application_new[NullablePointer[GObjectS]](name: Pointer[U8] tag, flags: I32)
use @g_application_activate[None](app: NullablePointer[GObjectS] tag)
...
use @g_application_register[I32](instance: NullablePointer[GObjectS] tag, a: Pointer[None], b: Pointer[None])
use @gtk_application_get_active_window[NullablePointer[GObjectS]](app: NullablePointer[GObjectS])
use @g_action_map_add_action_entries[None](action_map: NullablePointer[GObjectS] tag, entries: Pointer[None] tag, n_entries: I32, user_data: Any tag)
...
actor GtkApplication is GtkApplicationInterface
  ...
  be run() =>
    if (initialized) then
      if (@gtk_application_get_active_window(ptr).is_none()) then
        return
      else
      ...
      if (ActorPinning.is_successfully_pinned(auth)) then
        ...
        @g_signal_connect_data(ptr, "activate".cstring(),
            @{(gtkappptr: NullablePointer[GObjectS], ponygtk: GtkPony) =>
               ponygtk.activate()
  ...
  fun _final() =>
    ifdef debug then @printf("GtkApplication._final()\n".cstring()) end
      GObject.unref(ptr)
  be register_window_ptr(windowptr: NullablePointer[GObjectS] tag) =>
